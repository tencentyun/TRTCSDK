...
class ScreenEntranceViewController: UIViewController {
    ...
    func setupUI() {
        title = "屏幕分享"
        self.view.backgroundColor = UIColor.init(red: 0.14, green: 0.14, blue: 0.14, alpha: 1)
        ...
        recordScreenButton.backgroundColor = UIColor.init(red: -0.32, green: 0.66, blue: 0.4, alpha: 1)
        recordScreenButton.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.center
        recordScreenButton.addTarget(self, action: #selector(onTapRecordButton), for: UIControl.Event.touchUpInside)
        ...
        muteButton.setImage(UIImage.init(named: "rtc_mic_off"), for: UIControl.State.normal)
        muteButton.tag = 0
        muteButton.addTarget(self, action: #selector(onTapMuteButton), for: UIControl.Event.touchUpInside)
...
extension ScreenEntranceViewController {
    ...
    @objc func onTapMuteButton(button: UIButton) {
        if button.tag == 0 {
            print("开始静音")
            button.setImage(UIImage.init(named: "rtc_mic_on"), for: UIControl.State.normal)
            button.tag = 1
            trtcCloud.stopLocalAudio()
        } else if button.tag == 1 {
            print("关闭静音")
            button.setImage(UIImage.init(named: "rtc_mic_off"), for: UIControl.State.normal)
            button.tag = 0
...
extension  ScreenEntranceViewController : TRTCCloudDelegate {
