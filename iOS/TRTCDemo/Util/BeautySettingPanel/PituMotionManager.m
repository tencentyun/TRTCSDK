...
@implementation PituMotionManager
{
    NSMutableDictionary<NSString *, PituMotion *> *_map;
}

+ (instancetype)sharedInstance
{
    static PituMotionManager *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[PituMotionManager alloc] init];
    });
...
- (instancetype)init
{
    ...
    if (self) {
        NSArray *initList = @[
            @[@"video_3DFace_alalei0", @"http://dldir1.qq.com/hudongzhibo/AISpecial/ios/160/video_3DFace_alalei0.zip", L(@"阿拉蕾")],
        ...
        NSArray *(^generate)(NSArray *) = ^(NSArray *inputArray){
            ...
            for (NSArray *item in inputArray) {
                PituMotion *address = [[PituMotion alloc] initWithId:item[0] name:item[2] url:item[1]];
