...
|-----|-----|-----|-----|-----|
| sdkAppId | int | 是| 应用标识，腾讯云基于 sdkAppId 完成计费统计。 | 在 [实时音视频控制台](https://console.cloud.tencent.com/rav/) 创建应用后可在帐号信息页面中得到该 ID。 |
...
| roomId | int | 是|房间号码，在同一个房间内的用户可以看到彼此并进行视频通话。 | 您可以自定义设置该参数值，但不可重复。如果您的用户帐号 ID （userId）为数字类型，可直接使用创建者的用户 ID 作为 roomId。 |
| role | [TRTCRoleType] |  否| 直播场景下的角色，仅适用于直播场景（TRTCAppSceneLIVE），视频通话场景下指定无效。 | 默认值：主播（TRTCRoleAnchor）。 |
...
|-----|-----|-----|-----|-----|
| videoResolution | [TRTCVideoResolution] | 视频分辨率。 |  <li>视频通话建议选择360 × 640及以下分辨率，resMode 选择 Portrait。</li><li>手机直播建议选择 540 × 960，resMode 选择 Portrait。</li><li>Window 和 iMac 建议选择 640 × 360 及以上分辨率，resMode 选择 Landscape。</li> | TRTCVideoResolution 默认只有横屏模式的分辨率，例如640 × 360。如需使用竖屏分辨率，请指定 resMode 为 Portrait，例如640 × 360结合 Portrait 则为360 × 640。 |
...
| videoBitrate | int | 视频上行码率。 | 推荐设置请参考本文件前半部分 TRTCVideoResolution 定义处的注释说明。 | 码率太低会导致视频中出现大量马赛克。 |
...
| 属性 | 类型 | 字段含义 | 推荐取值 | 特别说明 |
|-----|-----|-----|-----|-----|
| preference | [TRTCVideoQosPreference]|  弱网下选择“保清晰”或“保流畅”。 | - |  <li>弱网下保流畅：在遭遇弱网环境时，画面会变得模糊，且出现较多马赛克，但可以保持流畅不卡顿。</li><li>弱网下保清晰：在遭遇弱网环境时，画面会尽可能保持清晰，但可能容易出现卡顿。</li> |
| controlMode | [TRTCQosControlMode] | 视频分辨率（云端控制 - 客户端控制）。 | 云端控制。 | <li>Server 模式（默认）：云端控制模式，若无特殊原因，请直接使用该模式。</li><li>Client 模式：客户端控制模式，用于 SDK 开发内部调试，客户请勿使用。</li> |
...
|-----|-----|-----|
| ip | String | 服务器 IP 地址。 |
| quality | [TRTCQuality]| 网络质量，内部通过评估算法测算出的网络质量，loss 越低，rtt 越小，得分便越高。 |
...
| videoGOP | int | 最终转码后的视频分辨率的关键帧间隔（又称为 GOP），单位秒。 | 3 |
...
| mixUsersArraySize | int | 数组 mixUsersArray 的大小。 | - |
...
| streamType | [TRTCVideoStreamType] | 流类型（大画面 &#124、小画面 &#124或辅路画面）。 |
...
| finalLoss | int | 该线路的总丢包率（％）<br>这个值越小越好，例如，丢包率为0表示网络很好。丢包率是该线路的 userId 从上行到服务器再到下行的总丢包率。如果 downLoss 为 0%，但是 finalLoss 不为0，说明该 userId 上行时出现了无法恢复的丢包。 |
| width | int | 视频宽度。 |
| height | int | 视频高度。 |
...
| rtt | int | 延迟（毫秒），指 SDK 到腾讯云服务器的一次网络往返时间，该小越好。一般低于50ms的 rtt 相对理想，而高于100ms的 rtt 会引入较大的通话延时。由于数据上下行共享一条网络连接，所以 local 和 remote 的 rtt 相同。 |
| receivedBytes | int | 总接收字节数（包含信令和音视频）。 |
| sentBytes | int | 总发送字节总数（包含信令和音视频）。 |
| localStatisticsArray | [TRTCLocalStatistics] | 本地的音视频统计信息，可能有主画面、小画面以及辅路画面等多路的情况，因此参数值为一个数组。 |
| localStatisticsArraySize | int | 数组 localStatisticsArray 的大小。 |
| remoteStatisticsArray | [TRTCRemoteStatistics]| 远端成员的音视频统计信息，可能有主画面、小画面以及辅路画面等多路的情况，因此参数值为一个数组。 |
| remoteStatisticsArraySize | int | 数组 remoteStatisticsArray 的大小。 |
