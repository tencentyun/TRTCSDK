...
public class TRTCRemoteUserManager {
    ...
    public void remoteUserVideoAvailable(String userId, int streamType, TXCloudVideoView renderView) {
        ...
        // 增加成员
        if (RemoteUserConfigHelper.getInstance().getRemoteUser(userId) == null) {
            RemoteUserConfig remoteUserConfig = new RemoteUserConfig(userId, streamType);
    ...
    public void updateCloudMixtureParams() {
        ...
        int resolution = ConfigHelper.getInstance().getVideoConfig().getVideoResolution();
        switch (resolution) {
