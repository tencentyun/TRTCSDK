    ...
    <div :class="{ 'audio-conference': true, 'is-show': isShowAudioCall }">
      ...
      <div class="audio-conference-list">
        ...
        >
          ...
          <!-- <div class="audio-item-avatar-wrapper">
            <img :src="userId2User[userId] && userId2User[userId].avatar" />
          </div>-->
        </div>
      </div>
...
export default {
  ...
  destroyed() {
    ...
    if (this.callStatus === "connected") {
      this.$trtcCalling.hangup({ status: "connected" });
  ...
  methods: {
    ...
    handleCancelCallUser: function() {
      this.$trtcCalling.hangup({ status: "calling" });
    ...
    startMeeting: function() {
      if (this.meetingUserIdList.length >= 3) {
        ...
        console.log("aaaaa", document.querySelector(`#audio-${lastJoinUser}`));
        this.$trtcCalling.startRemoteView({
          userID: lastJoinUser,
          videoViewDomID: `audio-${lastJoinUser}`
        });
        return;
      }
      ...
      this.$trtcCalling.startLocalView({
        ...
        videoViewDomID: `audio-${this.loginUserInfo.userId}`
      });
