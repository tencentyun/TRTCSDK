...
const DynamicRtc = dynamic(import('@components/RtcClient/improve-add-remove-audio-rtc-client'), { ssr: false });
...
export default function BasicRtc(props) {
  ...
  const [isAddedTrack, setIsAddedTrack] = useState(false);
  ...
  const handleAddTrack = async () => {
    await RTC.addAudioTrack();
  };

  const handleRemoveTrack = async () => {
    await RTC.removeAudioTrack();
  };
          ...
          <Accordion className={styles['accordion-container']} defaultExpanded={true}>
            ...
            <AccordionDetails className={styles['accordion-details-container']}>
              ...
              <div className={clsx(styles['button-container'], isMobile && styles['mobile-device'])}>
                ...
                <Button id="publish" variant="contained" color="primary" className={ isAddedTrack ? styles.forbidden : '' } onClick={handleAddTrack}>ADD AUDIO TRACK</Button>
                <Button id="unpublish" variant="contained" color="primary" className={ (isJoined && !isAddedTrack) ? styles.forbidden : '' } onClick={handleRemoveTrack}>REMOVE AUDIO TRACK</Button>
              </div>
            </AccordionDetails>
          </Accordion>
          {/* 用户列表 */}
...
export const getStaticProps = () => {
  const result = getNavConfig('improve-add-remove-audio');
  return {
    props: {
      ...result,
    },
