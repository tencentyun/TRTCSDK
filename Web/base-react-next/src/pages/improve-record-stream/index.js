...
const recordStartMode = 'START';
const recordStopMode = 'STOP';
...
export default function BasicRtc(props) {
  ...
  const [recordMode, setRecordMode] = useState(recordStartMode);
  const [recordMimeType, setRecordMimeType] = useState('');
  const [mimeTypeList, setMimeTypeList] = useState([]);
  const [recordPlaySource, setRecordPlaySource] = useState('');
  ...
  // 远端流变化时, 设置 Recording format
  useEffect(() => {
    const mimeTypeList = getSupportedMimeTypes();
    setMimeTypeList(mimeTypeList);
  }, [remoteStreamConfigList.length]);
  ...
  // 录制远端媒体流列表中的第一个媒体流中的音频
  const recordStart = () => {
    if (!recordMimeType) {
      toast.info(a18n('请选择录制格式'), 2000);
      return;
    }
    if ((remoteStreamConfigList || []).length === 0) {
      toast.info(a18n('远端流为空，无法录制'), 2000);
      return;
    }
    if (recordMode === recordStartMode) {
      ...
      // 绑定 onstop 事件(录制结束时触发)
      mediaRecorder.onstop = (event) => {
        console.log('Recorder stopped: ', event);
        console.log('Recorded Blobs: ', recordedBlobs);
      };
      // 绑定 ondataavailable 事件, 录制结束时同时触发，事件对象中返回录制的媒体数据
      mediaRecorder.ondataavailable = handleDataAvailable;
      mediaRecorder.start();
      setRecordPlaySource('');
      console.log('MediaRecorder started', mediaRecorder);
    } else {
      mediaRecorder && typeof mediaRecorder.stop === 'function' && mediaRecorder.stop();
    }
    setRecordMode(recordMode === recordStartMode ? recordStopMode : recordStartMode);
  };

  const recordPlay = () => {
    if (!isRecorded()) return;
    const [mimeType] = recordMimeType.split(';', 1);
    const superBuffer = new Blob(recordedBlobs, { type: mimeType });
    setRecordPlaySource(window.URL.createObjectURL(superBuffer));
  };
  ...
  const isRecorded = () => {
    if (recordMode === recordStopMode) { // can not playing on recording
      return false;
    }
    if ((recordedBlobs || []).length === 0) {
      toast.info(a18n('录制内容为空，请先录制'), 2000);
      return false;
    }
    return true;
  };
          ...
          <Accordion className={styles['accordion-container']} defaultExpanded={true}>
            ...
            <AccordionDetails className={styles['accordion-details-container']}>
              ...
              <Select value={recordMimeType} labelId="record-format" className={styles['select-container']} onChange={event => setRecordMimeType(event.target.value)}>
                {mimeTypeList.map((str, index) => <MenuItem key={`${index}-${str}`} value={str}>{str}</MenuItem>)}
              </Select>
              ...
              <div className={clsx(styles['button-container'], isMobile && styles['mobile-device'])}>
                ...
                <Button id="record" variant="contained" color="primary" onClick={recordStart}>
                  {recordMode === recordStartMode ? 'START' : 'STOP'} RECORD
                </Button>
                <Button id="play" variant="contained" color="primary" className={ recordMode === recordStopMode ? styles.forbidden : '' } onClick={recordPlay}>PLAY RECORD</Button>
                <Button id="play" variant="contained" color="primary" className={ recordMode === recordStopMode ? styles.forbidden : '' } onClick={recordDownload}>DOWNLOAD RECORD</Button>
              </div>

              {recordPlaySource
                && <video className={styles['video-container']} src={recordPlaySource} playsInline loop width="260" height="200" controls autoPlay></video>
              }
            </AccordionDetails>
          </Accordion>
          {/* 用户列表 */}
...
export const getStaticProps = () => {
  const result = getNavConfig('improve-record-stream');
  return {
    props: {
      ...result,
    },
