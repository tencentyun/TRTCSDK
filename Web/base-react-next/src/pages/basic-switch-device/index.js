...
const DynamicRtc = dynamic(import('@components/RtcClient/basic-switch-device-rtc-client'), { ssr: false });
...
export default function BasicRtc(props) {
  ...
  const switchCamera = () => {
    RTC && RTC.switchDevice('video', cameraID);
  };

  const switchMicrophone = () => {
    RTC && RTC.switchDevice('audio', microphoneID);
  };
        ...
        <Accordion className={styles['accordion-container']} defaultExpanded={true}>
          ...
          <AccordionDetails className={styles['accordion-details-container']}>
            ...
            <div className={clsx(styles['button-container'], isMobile && styles['mobile-device'])}>
              ...
              <Button id="publish" variant="contained" color="primary" onClick={switchCamera}>SWITCH CAMERA</Button>
              <Button id="unpublish" variant="contained" color="primary" onClick={switchMicrophone}>SWITCH MICROPHONE</Button>
            </div>
          </AccordionDetails>
        </Accordion>
        {/* 用户列表 */}
  ...
  return (
    <div className={clsx(styles['page-container'], isMobile && styles['mobile-device'])}>
      <Head>
        {mountFlag && <title>{a18n`${a18n(props.activeTitle)}-TRTC 腾讯实时音视频`}</title>}
...
export const getStaticProps = () => {
  const result = getNavConfig('basic-switch-device');
  return {
    props: {
      ...result,
    },
