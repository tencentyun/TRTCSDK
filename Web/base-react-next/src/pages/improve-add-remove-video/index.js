...
const DynamicRtc = dynamic(import('@components/RtcClient/improve-add-remove-video-rtc-client'), { ssr: false });
          ...
          <Accordion className={styles['accordion-container']} defaultExpanded={true}>
            ...
            <AccordionDetails className={styles['accordion-details-container']}>
              ...
              <div className={clsx(styles['button-container'], isMobile && styles['mobile-device'])}>
                ...
                <Button id="publish" variant="contained" color="primary" className={ isAddedTrack ? styles.forbidden : '' } onClick={handleAddTrack}>ADD VIDEO TRACK</Button>
                <Button id="unpublish" variant="contained" color="primary" className={ (isJoined && !isAddedTrack) ? styles.forbidden : '' } onClick={handleRemoveTrack}>REMOVE VIDEO TRACK</Button>
              </div>
            </AccordionDetails>
          </Accordion>
          {/* 用户列表 */}
...
export const getStaticProps = () => {
  const result = getNavConfig('improve-add-remove-video');
  return {
    props: {
      ...result,
    },
