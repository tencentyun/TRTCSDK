...
import { Button, Accordion, AccordionSummary, AccordionDetails, Typography, TextField, makeStyles } from '@material-ui/core';
...
export default function BasicRtc(props) {
  ...
  const handleStartPublishCDNStream = async () => {
    await RTC.handleStartPublishCDNStream();
  };

  const handleStopPublishCDNStream = async () => {
    await RTC.handleStopPublishCDNStream();
  };
        ...
        <div className={clsx(styles['body-container'], isMobile && styles['mobile-device'])}>
          <Accordion className={styles['accordion-container']} defaultExpanded={true}>
            ...
            <AccordionDetails className={styles['accordion-details-container']}>
              ...
              <TextField className={clsx(classes['new-input'], isMobile && classes['new-input-mobile'])} id="streamID" label="streamID" onChange={event => setStreamID(event.target.value)}/>
              <TextField className={clsx(classes['new-input'], isMobile && classes['new-input-mobile'])} id="appID" label="appID" onChange={event => setAppID(event.target.value)}/>
              <TextField className={clsx(classes['new-input'], isMobile && classes['new-input-mobile'])} id="bizID" label="bizID" onChange={event => setBizID(event.target.value)}/>
              <TextField className={clsx(classes['new-input'], isMobile && classes['new-input-mobile'])} id="publishCDNUrl" label="publishCDNUrl" onChange={event => setPublishCDNUrl(event.target.value)}/>
              ...
              <div className={clsx(styles['button-container'], isMobile && styles['mobile-device'])}>
                ...
                <Button id="publishTencentCDN" variant="contained" color="primary" onClick={handleStartPublishCDNStream}>START PUBLISH CDN</Button>
                <Button id="unpublishCDN" variant="contained" color="primary" onClick={handleStopPublishCDNStream}>STOP PUBLISH CDN</Button>
              </div>
            </AccordionDetails>
          </Accordion>
          {/* 用户列表 */}
...
export const getStaticProps = () => {
  const result = getNavConfig('improve-publishCDNStream');
  return {
    props: {
      ...result,
    },
