...
const DynamicRtc = dynamic(import('@components/RtcClient/improve-replace-track-rtc-client'), { ssr: false });
...
export default function BasicRtc(props) {
  ...
  const handleReplaceTrack = async (type) => {
    if (type === 'audio') {
      await RTC.replaceAudioTrack();
      if (localStreamConfig.mutedAudio) {
        RTC.muteAudio();
      }
    } else if (type === 'video') {
      await RTC.replaceVideoTrack();
          ...
          <Accordion className={styles['accordion-container']} defaultExpanded={true}>
            ...
            <AccordionDetails className={styles['accordion-details-container']}>
              ...
              <div className={clsx(styles['button-container'], isMobile && styles['mobile-device'])}>
                ...
                <Button id="publish" variant="contained" color="primary" onClick={() => handleReplaceTrack('audio')}>REPLACE AUDIO TRACK</Button>
                <Button id="unpublish" variant="contained" color="primary" onClick={() => handleReplaceTrack('video')}>REPLACE VIDEO TRACK</Button>
              </div>
            </AccordionDetails>
          </Accordion>
          {/* 用户列表 */}
...
export const getStaticProps = () => {
  const result = getNavConfig('improve-replace-track');
  return {
    props: {
      ...result,
    },
