...
void VodPlayerViewController::InitVodPlayerView() {
    ...
    TRTCCloudCore::GetInstance()->getTRTCCloud()->enableMixExternalAudioFrame(
        CDataCenter::GetInstance()->vod_push_, CDataCenter::GetInstance()->append_audio_play);

    TRTCCloudCore::GetInstance()->getTRTCCloud()->enableCustomVideoCapture(
        TRTCVideoStreamTypeSub, CDataCenter::GetInstance()->vod_push_);
...
void VodPlayerViewController::UnitVodPlayerView() {
    ...
    TRTCCloudCore::GetInstance()->getTRTCCloud()->enableMixExternalAudioFrame(false, false);
    TRTCCloudCore::GetInstance()->getTRTCCloud()->enableCustomVideoCapture(TRTCVideoStreamTypeSub,
                                                                           false);
    if (m_pVodPlayer)
    {
        m_pVodPlayer->stop();
...
void VodPlayerViewController::DoVodPlayerStarted(uint64_t msLength) {
    ...
    m_pPLAYSEEK->SetMaxValue(msLength);
}
