...
void TRTCMainViewController::onExitRoom(int reason)
{
    ...
    //pLogin->Create(NULL, _T("TRTCApp"), WS_VISIBLE | WS_SYSMENU | WS_CLIPCHILDREN, WS_EX_WINDOWEDGE);
    pLogin->Create(NULL, _T("TRTCApp"), WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, WS_EX_WINDOWEDGE);
    pLogin->CenterWindow();
...
void TRTCMainViewController::onViewBtnClickEvent(int id, std::wstring userId, int streamType)
{
    if (id == UI_EVENT_MSG::UI_BTNMSG_ID_MuteVideo)
    {
        ...
        if (localUserId.compare(userId) == 0)
        {
            ...
            {
                CMsgWnd::ShowMessageBox(GetHWND(), _T("TRTCApp Demo"), _T("Error: 未检出到摄像头，请检查本地电脑设备。"), 0xFFF08080);
                return;
            }
    ...
    else if (id == UI_EVENT_MSG::UI_BTNMSG_ID_MuteAudio)
    {
        ...
        if (localUserId.compare(userId) == 0)
        {
            ...
            if (deviceInfo.size() <= 0)
            {
                CMsgWnd::ShowMessageBox(GetHWND(), _T("TRTCApp Demo"), _T("Error: 未检出到麦克风，请检查本地电脑设备。"), 0xFFF08080);
                return;
            }
...
void TRTCMainViewController::DoExitRoom()
{
    ...
    {
        if (MSGID_OK == CMsgWnd::MessageBox(m_hWnd, _T("TRTCApp Demo"), _T("您确定要退出房间吗？")))
        {
            bExit = true;
