...
LRESULT MainViewBottomBar::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool & bHandled)
{
    if (uMsg == WM_MENUCLICK)
    {
        ...
        else if (sText == _T("音频设置"))
        {
            ...
            m_pSettingWnd->Create(m_pMainWnd->GetHWND(), _T("TRTCApp"), WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, WS_EX_WINDOWEDGE);
    ...
    else if (uMsg == ID_DELAY_SHOW_MSGBOX)
    {
        ...
        CMsgWnd::ShowMessageBox(m_pMainWnd->GetHWND(), _T("TRTCApp Demo"), text->c_str(), 0xFFF08080);
        delete text;
        text = nullptr;
    }
